"""Week 4 M2 reference: proves gateway<->client WebSocket wiring works.

NOT wired into `app` in elder_app.py -- kept here as a working reference
for whoever picks up wiring the real UI shell (elder_app.py) to the real
gateway. That's real, separate work: elder_app.py's `messages` state
models multiple independent per-contact conversation threads, but
`gateway/ws.py`'s WebSocket relay currently implements exactly one shared
broadcast circle -- no per-contact routing exists on the backend yet.
Making the real UI's chat screen actually send/receive needs that backend
work done first (or a deliberate decision to launch with one shared
circle instead of per-contact threads), not just swapping this module in.

What to actually carry forward, once that's decided: the connect/session/
reconnect logic below (CLIENT_JS), not this page's layout -- the layout
was always meant to be replaced by elder_app.py, which now has.
...
Verified working end-to-end, twice: once locally (two browser tabs,
cross-origin CORS, a hard gateway-container kill/restart with real
captured reconnect-backoff logs), and again against the real deployed
college server (join, live multi-user messaging, a real kill/restart of
the deployed gateway container with the same reconnect behavior). See
docs/prompt-journal.md's Week 4 entries for the full logs from both runs.
"""
...
def index() -> rx.Component:
    """Not routed anywhere -- reference only. See module docstring."""
    return rx.el.div(
        rx.el.h1(
        ...
        rx.el.p(
            "Gateway WS wiring proof (Week 4 M2) — not routed into the app; "
