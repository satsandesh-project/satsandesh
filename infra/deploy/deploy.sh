...
if command -v ufw >/dev/null 2>&1; then
  ...
  # talks over the compose-internal network. (8008/Tuwunel and
  # 8101/matrix-circle-service used to be listed here too; both
  # services were retired from docker-compose.yml after ADR 0002's
  # 2026-09-08 reversal to Option B.)
...
echo "Starting the base stack (gateway, elder-app, ai-services, caddy," \
     "postgres). No profile: the matrix profile was retired after ADR" \
     "0002's 2026-09-08 reversal to Option B (services/gateway/ on" \
     "Postgres is the backbone) -- see docs/adr/0002-chat-backbone.md..."
docker compose up -d
