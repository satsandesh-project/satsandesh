...
def _require_hs_token(authorization: Optional[str]) -> None:
...
async def receive_transaction(
    txn_id: str, request: Request, authorization: Optional[str] = Header(default=None)
):
...
async def proxy_keys_query(authorization: Optional[str] = Header(default=None)):
...
async def query_user(user_id: str, authorization: Optional[str] = Header(default=None)):
    _require_hs_token(authorization)
...
async def query_room(room_alias: str, authorization: Optional[str] = Header(default=None)):
    _require_hs_token(authorization)
